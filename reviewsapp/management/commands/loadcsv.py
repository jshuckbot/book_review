    ...
    def handle(self, *args, **options):
        ...
        for data_dict in models.get("Publisher", []):
            p, created = Publisher.objects.get_or_create(
                ...
                defaults={"website": data_dict["publisher_website"], "email": data_dict["publisher_email"]},
            )
            ...
            if created:
                print(
                    'Created Contributor "{} {}"'.format(
                        data_dict["contributor_first_names"], data_dict["contributor_last_names"]
                    )
                )
        ...
        for data_dict in models.get("BookContributor", []):
            ...
            bc, created = BookContributor.objects.get_or_create(
                book=book, contributor=contributor, role=data_dict["book_contributor_role"]
            )
            if created:
